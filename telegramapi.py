        ...
        if u.exists():
            ...
            if webhook['text'] == "وضعیت حساب":
                ...
                if len(Loan_queue.objects.filter(bankaccount__in=bk, status=0)):
                    new_lo=new_loan.objects.get(loan_queue__in=Loan_queue.objects.filter(bankaccount__in=bk, status=0))
                    ...
                    mess += "مبلغ پرداخت شده توسط شما:" + locale.currency(Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*1000 - Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*1000 - (new_lo.peak*1000*CountPayLoan(new_lo)), grouping=True)+"\n"
