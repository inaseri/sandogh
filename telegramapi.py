            ...
            if webhook['text'] == "وضعیت حساب":
                ...
                if len(Loan_queue.objects.filter(bankaccount__in=bk, status=0)):
                    ...
                    for new_lo in new_los:
                        mess +="---------------\n"
                        mess +="شماره حساب:"+str(new_lo.loan_queue.bankaccount.name)+"\n"
                        ...
                        mess += "مبلغ کل وام:" + locale.currency(Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*10000, grouping=True )+"\n"
                        mess += "مبلغ هر قسط:" + locale.currency(new_lo.peak*10000, grouping=True)+"\n"
                        mess += "مبلغ باقیمانده:" + locale.currency(Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*10000 - (new_lo.peak*10000*CountPayLoan(new_lo)), grouping=True)+"\n"
                        mess += "مبلغ پرداخت شده توسط شما:" + locale.currency(-1 * (Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*10000 - Loan_queue.objects.filter(bankaccount__in=bk, status=0)[0].amount*10000 - (new_lo.peak*10000*CountPayLoan(new_lo))), grouping=True)+"\n"
